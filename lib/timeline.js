 ...
 * its chronological neighbours. Existing overlapping data is left editable
 * instead of trapping the user in an impossible range.
 */
...
export function hasCueOverlap(cues, cueId) {
  ...
  if (!cue) return false;
  return cues.some(
    (item) => item.id !== cueId && cue.start < item.end && cue.end > item.start,
  );
}
