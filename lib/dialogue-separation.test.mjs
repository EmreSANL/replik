...
test('the local cinematic worker uses the same durable flow and authenticates its output request', async () => {
  const f = fixture();
  const options = { engine: 'cinematic-cdx23-ensemble-v2', apiBase: 'http://127.0.0.1:8011', fetcher: async (url, init) => {
    if (url.endsWith('/health')) return Response.json({ status: 'ok', engine: 'cinematic-cdx23-ensemble-v2' });
    assert.equal(init.headers['x-api-key'], 'server-key');
...
test('an older cinematic worker cannot cache a new-version background', async () => {
  const f = fixture();
  await assert.rejects(
    f.run({
      engine: 'cinematic-cdx23-ensemble-v2',
      apiBase: 'http://127.0.0.1:8011',
      fetcher: async () => Response.json({ status: 'ok', engine: 'cinematic-cdx23' }),
    }),
    /servisi güncellenmeli/,
  );
  assert.equal(f.jobs.size, 0);
});
